...
public class TimelineBoard extends JPanel implements ActionListener {
    ...
    private Map<GeneType, Timeline> timelines = new HashMap<>();
    ...
    private void fillTimelines() {
        ...
        List<GeneType> geno = animals.get(0).getGenotype().getGeneTypes();

        double hue = 0.15;
        final double hueChange = .3;
        for (GeneType type : geno) {
            addTimeline(new Timeline(type.toString(), Color.getHSBColor((float) hue, .5f, .5f)), type);
            hue += hueChange;
        }
    }

    public void addTimeline(Timeline timeline, GeneType type) {
    ...
    private void updateTimelines() {
        List<Animal> animals = arena.getAllAnimals();
        Map<GeneType, Double> bigmap = new HashMap<>();

        for (GeneType type : timelines.keySet()) {
            bigmap.put(type, 0.);
        }

        for (Animal ani : animals) {
            for (GeneType type : timelines.keySet()) {
                bigmap.put(type, bigmap.get(type) + ani.getGenotype().getGene(type));
            }
        }

        for (Entry<GeneType, Double> entry : bigmap.entrySet()) {
            Timeline myTimeline = getTimeline(entry.getKey());
            myTimeline.addPoint(entry.getValue() / animals.size());
        }
    ...
    private Timeline getTimeline(GeneType name) {
